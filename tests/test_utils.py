...
from utils.utils import create_rnd_binary_vector, find_largest_size, find_smallest_size, extract_integer_from_bits
...
class Test(TestCase):
    ...
    def test_extract_integer_from_bits(self):
        ...
        first_nine_bits = extract_integer_from_bits(number, 0, 9)
        assert first_nine_bits == 0b010101110
        first_three_bits = extract_integer_from_bits(number, 0, 3)
        assert first_three_bits == 6
        assert first_three_bits == 0b110
        nine_bits_from_index1 = extract_integer_from_bits(number, 1, 9)
        assert nine_bits_from_index1 == 0b101010111
        three_bits_from_index_9 = extract_integer_from_bits(number, 9, 3)
        assert three_bits_from_index_9 == 1
        assert three_bits_from_index_9 == 0b001
        last_bit = extract_integer_from_bits(number, 14, 1)
        assert last_bit == 1
        assert last_bit == 0b1
